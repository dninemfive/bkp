...
{
    public static async Task<IoResult> TryCopyAsync(string oldFilePath, string newFilePath)
    {
    ...
    public static async Task<IoResult> TryDeleteAsync(string path)
    {
        if (!File.Exists(path))
        {
    ...
    public static async Task<IoResult> TryMove(string oldFilePath, string newFilePath)
    {
        IoResult result = await TryCopyAsync(oldFilePath, newFilePath);
        return result.category is not ResultCategory.Success ? result : await TryDeleteAsync(oldFilePath);
    }
}
